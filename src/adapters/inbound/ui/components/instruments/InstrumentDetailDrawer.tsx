...
function InfoField({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="text-xs uppercase tracking-wide text-ink-400 dark:text-ink-500">{label}</p>
      <p className="mt-0.5 font-ledger text-sm font-medium text-ink-900 dark:text-ink-50">{value}</p>
    </div>
  );
}
...
function HistoryRow({ entry, currency }: { entry: InstrumentHistoryEntry; currency: string }) {
  ...
  return (
    <li className="relative flex gap-3 pb-6 last:pb-0">
      <span className="absolute bottom-0 left-[15px] top-6 w-px bg-ink-100 dark:bg-ink-800 last:hidden" />
      ...
      <div className="min-w-0 flex-1 pt-0.5">
        <div className="flex flex-wrap items-baseline justify-between gap-x-2 gap-y-0.5">
          <p className="text-sm font-medium text-ink-900 dark:text-ink-50">{fieldLabels[entry.field]}</p>
          <p className="text-xs text-ink-400 dark:text-ink-500">{formatDateTime(entry.changedAt)}</p>
        </div>
        <p className="mt-0.5 text-xs text-ink-500 dark:text-ink-400">{sourceLabels[entry.source]}</p>
        {entry.field !== "created" && (
          <p className="mt-1 font-ledger text-sm text-ink-700 dark:text-ink-200">
        ...
        {entry.field === "created" && (
          <p className="mt-1 font-ledger text-sm text-ink-700 dark:text-ink-200">
            {t("instruments.history.initialPrice")} <span className="font-semibold text-ink-900 dark:text-ink-50">
              {formatAmount(entry.newValue, currency)}
            </span>
          </p>
        )}
    ...
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col bg-white dark:bg-ink-900 shadow-xl animate-drawer-in focus:outline-none"
          aria-describedby={undefined}
        >
          {instrument && (
            <>
              <div className="flex items-start justify-between gap-3 border-b border-ink-100 dark:border-ink-800 px-6 py-5">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <DialogPrimitive.Title className="truncate font-ledger text-lg font-semibold text-ink-900 dark:text-ink-50">
                      {instrument.symbol}
                  ...
                  <p className="mt-0.5 truncate text-sm text-ink-500 dark:text-ink-400">{instrument.name}</p>
                </div>
                <DialogPrimitive.Close className="flex-none rounded-md p-1 text-ink-400 dark:text-ink-500 transition hover:bg-ink-50 dark:hover:bg-ink-800 hover:text-ink-600 dark:hover:text-ink-300">
                  <X className="h-5 w-5" />
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5">
                <div className="grid grid-cols-2 gap-4 rounded-xl border border-ink-100 dark:border-ink-800 bg-ink-50/40 p-4">
                ...
                <div className="mt-5 flex items-center gap-4 text-xs text-ink-500 dark:text-ink-400">
                ...
                <div className="mt-6">
                  <div className="mb-3 flex items-center gap-1.5 text-sm font-semibold text-ink-900 dark:text-ink-50">
                    <HistoryIcon className="h-4 w-4 text-ink-400 dark:text-ink-500" />
                    {t("instruments.history.title")}
                  </div>

                  {isLoading && (
                    <p className="text-sm text-ink-400 dark:text-ink-500">{t("instruments.history.loading")}</p>
                  )}
                  {isError && (
                    <p className="text-sm text-rose-600">{getErrorMessage(error)}</p>
                  )}
                  {!isLoading && !isError && history && history.length === 0 && (
                    <p className="text-sm text-ink-400 dark:text-ink-500">{t("instruments.history.empty")}</p>
                  )}
                  {!isLoading && !isError && history && history.length > 0 && (
                    <ul>
                      {history.map((entry) => (
